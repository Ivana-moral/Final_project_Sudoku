...
class SudokuGenerator:
       ...
       #Load the Board
        ...
        for row in range(self.row_length):
            ...
            for col in range(self.row_length):
                cell_holder = cell.Cell(0, row, col, 0)
                row_holder.append(cell_holder)
            self.board.append(row_holder)
    ...
    '''
        ...
        Return: None
    '''
    def print_board(self, type='puzzle'):
        for row in range(self.row_length):
            for col in range(self.row_length):
                if type == 'solution':
                    print(self.board[row][col].solution, end=' ')
                else:
    ...
    def valid_in_row(self, row, num):
        for col in range(self.row_length):
            cell_holder = self.board[row][col]
            if cell_holder.value == num:
                return False
        return True
        pass
   ...
   '''
            ...
            for col in range(col_start, col_start + 3):
                while True:
                    ...
                    if self.valid_in_box(row_start, col_start, num):
                        cell_holder = self.board[row][col]
                        cell_holder.set_initial_value(num)
                        break
        pass
    ...
    def fill_remaining(self, row, col):
        ...
        for num in range(1, self.row_length + 1):
            if self.is_valid(int(row), int(col), num):
                cell_holder = self.board[int(row)][int(col)]
                cell_holder.set_initial_value(num)
                if self.fill_remaining(int(row), int(col) + 1):
                    return True
                cell_holder.set_initial_value(0)
        return False
